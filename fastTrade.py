...
def send_telegram_message():
    file_location = f"Logs/{symbol} {today} fastTrade.csv"
...
today = date.today()
header_list = ['Datetime', 'Symbol', 'Buy/Sell', 'Entry', 'Exit', 'Difference']
with open(f'Logs/{symbol} {today} fastTrade.csv', 'a', newline='') as csvfile:
    writer = csv.DictWriter(csvfile, delimiter=',', lineterminator='\n', fieldnames=header_list)
    writer.writeheader()
today = date.today()
logging.basicConfig(filename=f"Logs/{symbol} {today} fastTrade.log",
                        filemode='a',
...
def buy_sell_trade(buy_or_sell, last_traded_price, strike_price):
    ...
    if symbol == "NSE:FINNIFTY-INDEX":
        ...
        strike_name = "NSE:NIFTY23323{0}PE".format(strike_price)
        lot_size = 50
    ...
    strike_price_full_name = strike_name
    response = fyers.place_order(data)
    print(response)
    strike_price_buy_price = strike_price_ltp
    return strike_price, strike_price_full_name, strike_price_buy_price
...
def trade(symbol):
    global strike_price
    global strike_price_full_name
    global strike_price_buy_price
    profit = loss = 0.0
    start_time = "09:21:00"
    end_time = "15:10:00"
    ...
    trade_strike_price = {"Datetime": None, "Symbol": None, "Buy/Sell": None, "Entry": None, "Exit": None, "Difference": None}
    position = None

    while datetime.datetime.now().strftime("%H:%M:%S") > start_time and datetime.datetime.now().strftime("%H:%M:%S") < end_time:
    # while True:
            ...
            if position != "sell" and second_last_candle_high > second_last_ma_value and second_last_candle_low > second_last_ma_value and abs(
                ...
                if last_traded_price < second_last_candle_low:
                    if position != "sell":
                        ...
                        position = "sell"
                        strike_price, strike_price_full_name, strike_price_buy_price = buy_sell_trade("buy",last_traded_price,strike_price=None)
                        logging.info(df)
...
#   Exit trade
            if position == "sell":
                ...
                strike_price_ltp = get_ltp(strike_price_full_name)
                running_pnl = (float(strike_price_ltp) - float(strike_price_buy_price)) * 50
                print(running_pnl)
                if last_traded_price > stop_loss_price:
                    ...
                    strike_price = position = None
                    loss = loss + difference
                    trade_strike_price['Symbol'] = strike_price
                    trade_strike_price['Buy/Sell'] = "Sell"
                    trade_strike_price['Exit'] = strike_price_ltp
                    trade_strike_price['Difference'] = running_pnl
                    write_in_excel(trade_strike_price)
                elif running_pnl > 300:
                    buy_sell_trade("sell",last_traded_price, strike_price)
                    difference = float(trade["SL"]) - entry_price
                    print("1:3 Exit position at PROFIT : {0} at LTP : {1} EXIT : {2} TIME : {3}".format(
                        difference, last_traded_price, float(trade["SL"]), current_time))
                    trade = {"Datetime": None, "Symbol": None, "Buy/Sell": None, "Entry": None, "Exit": None,
                             "Exit Date": None, "SL": None}
                    strike_price = position = None
